...
export async function forceSyncPrayerBreakState(employeeId, setIsOnPrayerBreak, setPrayerBreakTimer, setLoadingPrayerBreak, setPrayerBreakIntervalId) {
  try {
    // Fetch ALL prayer breaks for this employee to find any open one (not just today)
    const res = await fetch(`/api/prayer_breaks?employeeId=${String(employeeId).trim()}`);
    const data = await res.json();
    
    if (data.success && Array.isArray(data.prayer_breaks) && data.prayer_breaks.length > 0) {
      // Find ANY running prayer break (prayer_break_start exists but prayer_break_end is NULL)
