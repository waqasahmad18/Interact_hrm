...
export async function forceSyncBreakState(employeeId, setIsOnBreak, setBreakTimer, setLoadingBreak, setBreakIntervalId) {
  try {
    // Fetch ALL breaks for this employee to find any open one (not just today)
    const res = await fetch(`/api/breaks?employeeId=${String(employeeId).trim()}`);
    const data = await res.json();
    
    if (data.success && Array.isArray(data.breaks) && data.breaks.length > 0) {
      // Find ANY running break (break_start exists but break_end is NULL)
