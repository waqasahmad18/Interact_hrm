...
export default function SmokeTestPage() {
  ...
  const [rows, setRows] = React.useState<SmokeRow[]>([]);
  ...
  const load = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/smoke-test", { cache: "no-store" });
      const data = await res.json();
      if (!res.ok || !data.success) throw new Error(data.error || "Failed to load");
      setRows(data.rows as SmokeRow[]);
    } catch (err) {
  ...
  return (
    <div style={{ maxWidth: 640, margin: "0 auto", padding: 24 }}>
      <h1 style={{ fontSize: 22, fontWeight: 700, marginBottom: 4 }}>Smoke Test</h1>
      <p style={{ color: "#6b7280", marginBottom: 20 }}>
        End-to-end check: this saves to the <code>hrm_smoke_test</code> table via
